...
#####################################################
def RjbMeanVar_MR(ndip=19, mindip=0.0, maxdip=np.pi/2.0,
    ...
    for i in range(0, nepi):
        for j in range(0, nmag):
            rjb_var, rjb_avg = EpsilonDipDxDyTheta(M[j], Repi[i],
            ...
            ratio[i,j] = rjb_avg / Repi[i]
            variance[i,j] = rjb_var
...
#    mindx = Repi / M / 2.0
            ...
            for k in range(0, ndip):
                ...
                for i in range(0, nx):
                    ...
                    for j in range(0, ny):
                        ...
                        Rjb2 = Rjb * Rjb
                        integrand.y[j] = one_over_2pi * np.trapz(Rjb, dx=dt)
                        integrand.y2[j] = one_over_2pi * np.trapz(Rjb2, dx=dt)
    ...
    if nw == 1:
        Rjb_avg = integrand.w[0]
        Rjb_var = integrand.w2[0] - Rjb_avg**2
    else:
        Rjb_avg = np.trapz(peps*integrand.w, dx=d_eps)
        Rjb_var = np.trapz(peps*integrand.w2, dx=d_eps) - Rjb_avg**2
...
def main():
    ...
    for iP in range(0, NP):
        if os.fork() == 0:
            RjbMeanVar_MR(rup_dim_model=rup_dim_model, mech=mech, LW=LW, AR=AR,
                          ndip=ndip, mindip=mindip, maxdip=maxdip,
