...
def RrupMeanVar_MR(ndip=19, mindip=0.0, maxdip=np.pi/2.0,
                   ...
                   neps=10, trunc=3,
                   NP=1, iP=0, filename='Repi-to-Rrup-',
                   M=6.0, Repi=100):
...
#    mindx = Repi / M / 2.0
                ...
                for z in range(0, nz):
                    for i in range(0, nx):
                        ...
                        for j in range(0, ny):
                            ...
                            integrand.y[j] = one_over_2pi * np.trapz(Rrup, dx=dt)
                            integrand.y2[j] = one_over_2pi * np.trapz(Rrup2, dx=dt)
    ...
    if nw == 1:
        Rrup_avg = integrand.w[0]
        Rrup_var = integrand.w2[0] - Rrup_avg**2
    else:
        Rrup_avg = np.trapz(peps*integrand.w, dx=d_eps)
        Rrup_var = np.trapz(peps*integrand.w2, dx=d_eps) - Rrup_avg**2
...
def main():
    ...
    for iP in range(0, NP):
        if os.fork() == 0:
            RrupMeanVar_MR(rup_dim_model=rup_dim_model, mech=mech, LW=LW, AR=AR,
                           ndip=ndip, mindip=mindip, maxdip=maxdip,
